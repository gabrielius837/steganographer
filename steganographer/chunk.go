...
func (chunk *Chunk) CalculateCrc() uint32 {
	var crc uint32 = 0xFFFFFFFF

	// start with header
	for i := 0; i < INT_SIZE; i++ {
		crc = computeCrcIteration(crc, chunk.header[i])
	}

	// end with data
	for i := 0; i < len(chunk.data); i++ {
		crc = computeCrcIteration(crc, chunk.data[i])
	}

	return ^crc
}

func (chunk *Chunk) IsValid() (bool, error) {
	if chunk.length != uint32(len(chunk.data)) {
		return false, fmt.Errorf("length and byte amount does not match for chunk, length: %d, bytes: %d", chunk.length, len(chunk.data))
	}

	computedCrc := chunk.CalculateCrc()
	if computedCrc != chunk.crc {
		return false, fmt.Errorf("computed and provided crc checksums do not match, computed: %d, expected: %d", computedCrc, chunk.crc)
	}

	return true, nil
}

func computeCrcIteration(crc uint32, octet byte) uint32 {
	return (crc >> 8) ^ TABLE[(byte(crc)&0xFF)^octet]
}
